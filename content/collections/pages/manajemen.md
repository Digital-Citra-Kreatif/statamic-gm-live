---
id: e326227e-7841-487a-af38-2bd38850278f
published: false
blueprint: page
title: Manajemen
template: manajemen
updated_by: 28d34247-1c17-42bf-8548-5b36f18adcbd
updated_at: 1787106790
featured_image: hero-manajemen.jpg
parent: home
sections:
  -
    id: ErB9NQ1tYFXI1Ejooh5GQ
    identifier: opening-management
    heading: 'Didukung Tim Profesional dan Berpengalaman'
    description:
      -
        type: paragraph
        content:
          -
            type: text
            text: 'GM Mobil didukung oleh tim profesional yang berpengalaman dalam industri kendaraan niaga untuk memastikan pelayanan, kualitas, dan dukungan operasional terbaik bagi setiap pelanggan.'
    show: true
    type: heading_description
    enabled: true
  -
    id: 3BhwuJOI8nDn6dltZDJIl
    image: frankie-makaminang.png
    name: 'Frankie Makaminang'
    role: 'Direktur Utama'
    bio:
      -
        type: paragraph
        content:
          -
            type: text
            text: 'Salam sejahtera bagi kita semua,'
      -
        type: paragraph
        content:
          -
            type: text
            text: 'Saya bersama segenap direksi dan seluruh tim Gaya Makmur Mobil akan terus berusaha meningkatkan pelayanan kami demi mendukung semua produk FAW Trucks yang sudah dan akan kami distribusikan di seluruh wilayah Indonesia, yang sudah mencapai ribuan unit sejak diluncurkan di Indonesia pada tahun 2009.'
      -
        type: paragraph
        content:
          -
            type: text
            text: 'Kami harapkan FAW Trucks akan terus menjadi pemimpin market truk Cina di Indonesia, seperti negeri asalnya RRC dan menjadi solusi alat transportasi dengan biaya investasi & operasi yang rendah.'
      -
        type: paragraph
      -
        type: paragraph
        content:
          -
            type: text
            text: 'Best Regards'
          -
            type: hardBreak
          -
            type: text
            text: 'Frankie Makaminang'
    background: default
    type: team_primary
    enabled: true
    identifier: section-highight-management
    show: true
  -
    id: R8my1k3qVbE4yybYz0-22
    heading: 'Board Of Directors'
    members:
      -
        id: 0KL6tq5wv05zGT8QwRRNm
        image: surijani-1.png
        name: Surijani
        role: 'Direktur Marketing'
      -
        id: J_dBB1VMnb1c5Yjc0PG2L
        image: inawati-1.png
        name: Inawati
        role: 'Direktur Keuangan'
    background: default
    type: team_grid
    enabled: true
    identifier: section-management-1
    show: true
  -
    id: '-397wH92kdJgCQ7cA75cV'
    heading: 'Board Of Commissioners'
    members:
      -
        id: IIF3OB9FRPZw_yCo5Gyc6
        image: tim-placeholder.png
        name: 'Lie Fen Sin'
        role: 'Komisaris Utama'
      -
        id: FeoiV3JxqB59oYxFUCc46
        image: tim-placeholder.png
        name: 'Cahyadi Lie'
        role: Komisaris
      -
        id: 6JnvDeVfnoeQQ11h99Qia
        image: tim-placeholder.png
        name: Hendry
        role: Komisaris
    background: default
    type: team_grid
    enabled: true
    identifier: section-management-2
    show: true
  -
    id: LBnB4pIe04CWlMAEgDJz5
    identifier: placeholder-tim
    section_images: tim-placeholder.png
    show: true
    type: team_placeholder
    enabled: true
---
